...
class Spotify(MiddleMan):
    ...
    def add_credits_artists_infos(album):
        ...
        for credit in album.credits:
            spotify_artist = Spotify.artists_cache[credit.artist.name]
            if not spotify_artist:
                spotify_artist = SpotifyApi.get_artist_info(credit.artist.name)
                Spotify.artists_cache[credit.artist.name] = spotify_artist
    ...
    def add_album_artists(album):
        ...
        for artist in album.artists:
            spotify_artist = Spotify.artists_cache[artist.name]
            if not spotify_artist:
                spotify_artist = SpotifyApi.get_artist_info(artist.name)
                Spotify.artists_cache[artist.name] = spotify_artist
